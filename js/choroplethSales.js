...
d3.json("data/myData/rankedData.json", function(data) {
    data.forEach(function(d) { newDict[d.id] = [+d.hopeDollars, d.COUNTY, d.sales];});
    ...
    d3.json("data/myData/simpleGA.json", function(json) {
        ...
        .data(json.features)
        .enter().append("path")
        .attr("class", function(d) { return quantize(newDict[d.id][2]);})
        .attr("d", path)
        .call(d3.helper.tooltip()
            //.attr({class: function(d, i) { return d + ' ' +  i + ' A'; }})
            .text(function(d){ return 'County: '+ newDict[d.id][1] + '<br />Lottery Ticket Sales: $' +commasFormatter(newDict[d.id][2]); })
        )
        ...
        .on("click", function(d) {
            $('#lotteryCounty').html(''+ newDict[d.id][1] +'')
            $('#lotteryInfo').html('$'+commasFormatter(newDict[d.id][2])+'')});
        /*.on("click", function(d) {
